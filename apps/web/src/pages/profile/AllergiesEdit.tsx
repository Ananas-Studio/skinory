...
function AllergiesEdit() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchProfile(userId), fetchAllergenOptions()])
      .then(([profile, opts]) => {
        if (cancelled) return
        setOptions(opts)
        const ids = new Set(profile.allergens.map((a) => a.id))
        setSelectedIds(ids)
        setInitialIds(ids)
      })
      .catch(console.error)
  ...
  const handleSave = useCallback(async () => {
    setSaving(true)
    try {
      await updateAllergens(userId, [...selectedIds])
      navigate(-1)
    } catch (err) {
  ...
  return (
    <div className="font-[Geist,'Avenir_Next','Segoe_UI',sans-serif] min-h-dvh bg-white">
      <div className="flex items-center gap-3 p-4 border-b border-[#f4f4f5]">
        ...
        <h1 className="text-lg font-semibold flex-1">Allergies & Sensitivities</h1>
        {hasChanges && (
      ...
      <div className="flex flex-col gap-5 p-4 pb-16">
        <p className="text-sm text-[#71717a]">Select ingredients you're sensitive or allergic to. These will be flagged during product analysis.</p>
        ...
        {grouped.map(([category, items]) => (
          <section key={category} className="flex flex-col gap-2.5">
            <h2 className="text-xs font-semibold text-[#71717a] uppercase tracking-wider">
              {(ALLERGEN_CATEGORY_LABELS as Record<string, string>)[category] ?? category}
            </h2>
            <div className="flex flex-wrap gap-2">
              {items.map((a) => (
                <ChipButton key={a.id} label={a.name} active={selectedIds.has(a.id)} onClick={() => toggle(a.id)} />
              ))}
            </div>
          </section>
        ))}
...
function groupByCategory(options: AllergenOption[]): [string, AllergenOption[]][] {
  const map = new Map<string, AllergenOption[]>()
